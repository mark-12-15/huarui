...
void BCSceneListWidgetData::Refresh()
{
    ...
    for (int i = 0; i < lstWindowSceneData.count(); i++) {
        ...
        if ( !lstGroupDisplay.contains( QString::number(0) ) ) {
            lstGroupDisplay.append( QString::number(0) );

            // 绘制房间
            if (NULL == pRoom) {
                pRoom = pApplication->GetMRoomByGroupDisplayID( 0 );
            }

            // 绘制屏组矩形
            if (NULL != pRoom) {
                BCMGroupDisplay *pGroupDisplay = pRoom->GetGroupDisplay( 0 );
        ...
        QString name;
        BCMChannel *pChannel = pApplication->GetInputChannel(pWindowSceneData->m_nChannelID, 0);
        if (NULL != pChannel) {
